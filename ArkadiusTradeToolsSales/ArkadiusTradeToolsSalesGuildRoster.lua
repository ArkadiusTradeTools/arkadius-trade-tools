...
function ArkadiusTradeToolsSales.GuildRoster:Enable(enable)
    if enable and (self.timeSelect == nil) then
        ...
        if not LGR then
            ...
            local purchasesColumn = LGR:AddColumn({
                key = "ATT_Purchases",
                header = {
                ...
                row = {
                    ...
                    format = function(value)
                        return ArkadiusTradeTools:LocalizeDezimalNumber(tonumber(value) or 0) .. " |t16:16:EsoUI/Art/currency/currency_gold.dds|t"
                    end,
                },
            })

            local salesColumn = LGR:AddColumn({
                key = "ATT_Sales",
                header = {
                    title = "Sales",
                    align = TEXT_ALIGN_RIGHT,
            ...
            LGR:SetBulkData({ purchasesColumn, salesColumn }, function(guildId, data, index)
                ...
                local purchases, sales = ArkadiusTradeToolsSales.GuildRoster:GetNumbers(guildName, data.displayName)
