...
function TooltipExtensionList:Initialize(control)
    ...
    ZO_ScrollList_AddDataType(self.list, 1, "ArkadiusTradeToolsSalesTooltipExtensionItemListRow", 16, function(control, data)
        self:SetupItemRow(control, data)
    end)
    ZO_ScrollList_AddDataType(self.list, 2, "ArkadiusTradeToolsSalesTooltipExtensionItemListEmptyRow", 16, function(control, data)
        self:SetupEmptyRow(control, data)
    end)
end
...
function TooltipExtensionList:SortScrollList()
    ...
    local Sort = function(entry1, entry2)
        local itemQuality1 = GetItemLinkQuality(entry1.data.itemLink)
        local itemQuality2 = GetItemLinkQuality(entry2.data.itemLink)
...
function ArkadiusTradeToolsSales.TooltipExtensions:Initialize(settings)
    ...
    if Settings.graphEnabled == nil then
        Settings.graphEnabled = true
    end
    if Settings.craftingEnabled == nil then
        Settings.craftingEnabled = true
    end
...
----------------------------------------------------------------------------
ArkadiusTradeToolsSales.TooltipExtension = ZO_InitializingObject:Subclass()
...
function ArkadiusTradeToolsSales.TooltipExtension:New(control, tooltip, days)
    ...
    object.SECONDS_IN_DAY = ZO_ONE_DAY_IN_SECONDS
    object.days = days or 10
...
function ArkadiusTradeToolsSales.TooltipExtension:Initialize()
    ...
    function self.tooltip.SetWornItem(tooltip, index, bagId)
        local itemLink = GetItemLink(bagId, index, LINK_STYLE_DEFAULT)
    ...
    function self.tooltip.SetAttachedMailItem(tooltip, id, index)
        local itemLink = GetAttachedItemLink(id, index, LINK_STYLE_DEFAULT)
    ...
    function self.tooltip.SetTradeItem(tooltip, tradeType, tradeIndex)
        local itemLink = GetTradeItemLink(tradeType, tradeIndex, LINK_STYLE_DEFAULT)
    ...
    function self.tooltip.SetTradingHouseListing(tooltip, tradingHouseListingIndex)
        local itemLink = GetTradingHouseListingItemLink(tradingHouseListingIndex, LINK_STYLE_DEFAULT)
    ...
    function self.tooltip.SetProvisionerResultItem(tooltip, recipeListIndex, recipeIndex)
        ...
        local itemLink = GetRecipeResultItemLink(recipeListIndex, recipeIndex, LINK_STYLE_DEFAULT)
        UpdateTooltip(tooltip, itemLink)
    ...
    for link, sales in pairs(itemSales) do
        ...
        if link == itemLink then
            ...
            for _, sale in pairs(sales) do
                ...
                if (price > maxPrice) and (price ~= math.huge) then
                    maxPrice = price
                end
            end
