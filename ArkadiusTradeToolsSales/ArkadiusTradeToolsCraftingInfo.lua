...
local GetItemStyleMaterialLink = GetItemStyleMaterialLink
...
function ArkadiusTradeToolsSales:GetMasterWritComponents(itemLink)
    ...
    if writType == nil then
        logger:Info(string_fmt("ATT: Unknown master writ item id %d", itemId))
        return {}
    end
    local components = {}
    local component
    ...
    if (writType == MASTER_WRIT_TYPE_BLACKSMITHING) or (writType == MASTER_WRIT_TYPE_TAILORING) or (writType == MASTER_WRIT_TYPE_WOODWORKING) or (writType == MASTER_WRIT_TYPE_JEWELRY) then
        ...
        if component.itemLink ~= nil and component.quantity ~= nil then
        ...
        if writType ~= MASTER_WRIT_TYPE_JEWELRY then
            component = { itemLink = GetItemStyleMaterialLink(style, LINK_STYLE_DEFAULT), quantity = 1 }
            if component.itemLink ~= nil and component.quantity ~= nil then
                components[index] = component
                index = index + 1
            else
