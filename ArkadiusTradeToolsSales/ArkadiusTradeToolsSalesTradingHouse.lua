...
local function SetUpSearchResultsWithoutAGS(rowControl)
    local profitMarginControl, averagePricePerUnitControl, averagePriceControl, nameControl, sellPricePerUnitControl, sellPriceControl, timeRemainingControl, purchasePricePerUnit, averagePrice, averagePricePerUnit = GetControlsForSearchResults(rowControl)
...
local function OnEvent(eventCode, responseType, result)
    if
        (eventCode == EVENT_TRADING_HOUSE_RESPONSE_RECEIVED)
        ...
        and TRADING_HOUSE.searchResultsList
    then
...
-- 	--[[ in case MM is loaded too]]
-- 	local lastSellPriceButton = buttonContainer:GetNamedChild("AveragePriceButton") or buttonContainer:GetNamedChild("LastSellPriceButton")
...
local function GetDenominator(itemLink, itemType)
    if itemType == ITEMTYPE_MASTER_WRIT then
        return ArkadiusTradeToolsSales:GetVoucherCount(itemLink)
    else
        return 1
    end
end

local function HandlePriceSelection(button)
    local itemLink = ArkadiusTradeToolsSales:NormalizeItemLink(AwesomeGuildStore.internal.tradingHouse.sellTab.pendingItemLink)
    local itemType = GetItemLinkItemType(itemLink)
    local denominator = GetDenominator(itemLink, itemType)
    local days = ArkadiusTradeToolsSalesData.settings.tooltips.days
    local price = ArkadiusTradeToolsSales:GetAveragePricePerItem(itemLink, GetTimeStamp() - SECONDS_IN_DAY * days) / denominator
    if price then
        AwesomeGuildStore.internal.tradingHouse.sellTab:SetUnitPrice(math.floor(price + 0.5))
...
function ArkadiusTradeToolsSales.TradingHouse:AddAGSPriceButton()
    ...
    averagePriceButton.control:ClearAnchors()
    averagePriceButton.control:SetAnchor(RIGHT, lastSellPriceButton, LEFT, 2, 0)
    averagePriceButton.control:SetDrawLayer(DL_OVERLAY)
    averagePriceButton.HandlePress = function(button)
...
function ArkadiusTradeToolsSales.TradingHouse.SetPendingItemPrice(_tradingHouse, slotId, isPending)
    if not Settings.enableAutoPricing then
        return
    end
    local _, stackCount = GetItemInfo(BAG_BACKPACK, slotId)
    local itemLink = ArkadiusTradeToolsSales:NormalizeItemLink(GetItemLink(BAG_BACKPACK, slotId, LINK_STYLE_DEFAULT))
    local days = ArkadiusTradeToolsSalesData.settings.tooltips.days
...
function ArkadiusTradeToolsSales.TradingHouse:Enable(enable)
    if enable and self.profitMarginDaysLabel == nil then
        ...
        if AwesomeGuildStore then
            ...
            ZO_PostHook(AwesomeGuildStore.class.SellTabWrapper, "InitializeListingInput", function()
                self:AddAGSPriceButton()
            end)
        else
...
function ArkadiusTradeToolsSales.TradingHouse.InitAGSIntegration(tradingHouseWrapper)
    ...
    function AGSFilter:Initialize()
        self.averagePrices = {}
        ValueRangeFilterBase.Initialize(self, SUBFILTER_ATT, FilterBase.GROUP_LOCAL, {
            label = "Deal Finder",
            min = MIN_VALUE,
            max = MAX_VALUE,
            steps = STEPS,
    ...
    EVENT_MANAGER:RegisterForEvent(ArkadiusTradeToolsSales.NAME, EVENT_CLOSE_TRADING_HOUSE, function()
        ArkadiusTradeToolsSales.TradingHouse.Filter:ResetCache()
    end)
end

function ArkadiusTradeToolsSales.TradingHouse:RegisterAGSInitCallback()
    if self.AGSRegistered then
        return
    end
    self.AGSRegistered = true
    AwesomeGuildStore:RegisterCallback(AwesomeGuildStore.callback.AFTER_FILTER_SETUP, self.InitAGSIntegration)
end
