local localization =
{
    ATT_STR_BUYER                    = "Comprador",
    ATT_STR_SELLER                   = "Vendedor",
    ATT_STR_GUILD                    = "Guilda",
    ATT_STR_ITEM                     = "Item",
    ATT_STR_UNIT_PRICE               = "Preço unitário",
    ATT_STR_PRICE                    = "Preço",
    ATT_STR_START_TIME               = "Hora de início",
    ATT_STR_END_TIME                 = "Hora de término",
    ATT_STR_GENERATED_TIME           = "Hora de criação",
    ATT_STR_EXPORTS                  = "Exportações",
    ATT_STR_EXPORT                   = "Exportar",
    ATT_STR_TODAY                    = "Hoje",
    ATT_STR_YESTERDAY                = "Ontem",
    ATT_STR_TWO_DAYS_AGO             = "Dois dias antes",
    ATT_STR_THIS_WEEK                = "Esta semana",
    ATT_STR_LAST_WEEK                = "Semana passada",
    ATT_STR_PRIOR_WEEK               = "Duas semanas atrás",
    ATT_STR_7_DAYS                   = "7 dias",
    ATT_STR_10_DAYS                  = "10 dias",
    ATT_STR_14_DAYS                  = "14 dias",
    ATT_STR_30_DAYS                  = "30 dias",
    ATT_STR_KEEP_EXPORTS_FOR_DAYS    = "Manter a exportação por X dias",
    ATT_STR_FILTER_TEXT_TOOLTIP      = "Filtro de texto para usuário, guilda ou por nome de item",
    ATT_STR_FILTER_SUBSTRING_TOOLTIP = "Muda a pesquisa da guia atual para a guia secundária ou vice-versa. Substitui os caracteres em letras maiúsculas em minúsculas.",
    ATT_STR_FILTER_COLUMN_TOOLTIP    = "Excluir/incluir esta coluna de/para filtro de texto",
    ATT_STR_EXPORT_RELOAD_WARNING    = "Exportação salva. Por favor/recarregue UI para gravar no disco.",
    ATT_STR_INCLUDE_ONLY_MEMBERS     = "Incluir apenas membros"
}

ZO_ShallowTableCopy(localization, ArkadiusTradeTools.Modules.Exports.Localization)