...
local function create_printer(get_parser_state)
    ...
    function printer.dump_internal_state(values)
        ...
        for _, entry in ipairs(required) do
            print("  " .. entry.key .. string.rep(" ", maxlabel + 2 - #entry.key) .. " => '" .. tostring(values[entry]) .. "'")
        end

        if optargument then
            print("\nOptional arguments:" .. optargument.key .. "; allowed are " .. tostring(optargument.maxcount) .. " arguments")

            if optargument.maxcount == 1 then
                print("  " .. optargument.key .. string.rep(" ", maxlabel + 2 - #optargument.key) .. " => '" .. optargument.key .. "'")
            else
                for i = 1, optargument.maxcount do
                    if values[optargument] and values[optargument][i] then
                        print("  " .. tostring(i) .. string.rep(" ", maxlabel + 2 - #tostring(i)) .. " => '" .. tostring(values[optargument][i]) .. "'")
                    end
                end
            end
        end
