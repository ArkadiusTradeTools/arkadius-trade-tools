...
local function attRound(num, numDecimals)
    return tonumber(string.format("%." .. (numDecimals or 0) .. "f", num))
end

ArkadiusTradeTools.attRound = attRound

local function RequestMoreGuildHistoryCategoryEventsLocal(guildIndex, category, numGuilds)
    if RequestMoreGuildHistoryCategoryEvents(GetGuildId(guildIndex), category, false) then
...
function ArkadiusTradeTools:Initialize()
    ...
    buttonForceRescan.OnClicked = function()
        self:FireCallbacks(EVENTS.ON_RESCAN_GUILDS)
    end
...
local ZO_ONE_MINUTE_IN_SECONDS = 60
local ZO_ONE_HOUR_IN_MINUTES = 60
local ZO_ONE_DAY_IN_HOURS = 24
local ZO_ONE_HOUR_IN_SECONDS = ZO_ONE_HOUR_IN_MINUTES * ZO_ONE_MINUTE_IN_SECONDS -- = 3600
local ZO_ONE_DAY_IN_SECONDS = ZO_ONE_DAY_IN_HOURS * ZO_ONE_HOUR_IN_SECONDS -- = 86400
local SECONDS_IN_WEEK = 604800
local TIME_FORMAT_STRING = "%Y-%m-%d"
local TIME_FORMAT_STYLE = TIME_FORMAT_STYLE_CLOCK_TIME
local TIME_FORMAT_PRECISION = TIME_FORMAT_PRECISION_TWENTY_FOUR_HOUR
local currentTimeStamp = GetTimeStamp()

---@return string|osdate
function ArkadiusTradeTools:GetLocalTimeShift()
    if not self.timeShift then
        local timeShift = GetSecondsSinceMidnight() - (currentTimeStamp % ZO_ONE_DAY_IN_SECONDS)
        if timeShift < -12 * ZO_ONE_HOUR_IN_SECONDS then
            timeShift = timeShift + ZO_ONE_DAY_IN_SECONDS
        end
        self.timeShift = timeShift
    end
    return os.date("%Y-%m-%d %H:%M:%S", currentTimeStamp + self.timeShift)
end
...
function ArkadiusTradeTools:TimeStampToDateTimeString(timeStamp)
    ...
    local dateString = os.date(TIME_FORMAT_STRING, timeStamp - timeShift)
    local timeString = ZO_FormatTime(timeStamp % ZO_ONE_DAY_IN_SECONDS, TIME_FORMAT_STYLE, TIME_FORMAT_PRECISION)
    return dateString .. " " .. timeString
end

function ArkadiusTradeTools:TimeStampToAgoString(timeStamp)
    return ZO_FormatDurationAgo(currentTimeStamp - timeStamp)
end

---@param timeStamp integer
---@return integer timeStamp
function ArkadiusTradeTools:GetStartOfDayFromTimeStamp(timeStamp)
    timeStamp = timeStamp or currentTimeStamp
    timeStamp = zo_floor(timeStamp / ZO_ONE_DAY_IN_SECONDS) * ZO_ONE_DAY_IN_SECONDS
    return timeStamp
end
...
function ArkadiusTradeTools:GetStartOfHourFromTimeStamp(timeStamp)
    timeStamp = timeStamp or currentTimeStamp
    timeStamp = zo_floor(timeStamp / ZO_ONE_HOUR_IN_SECONDS) * ZO_ONE_HOUR_IN_SECONDS
    return timeStamp
end

--- Returns the UTC timestamp for last midnight ---
---@param relativeDay any
---@return integer timeStamp
function ArkadiusTradeTools:GetStartOfDay(relativeDay)
    relativeDay = relativeDay or 0
    local days = zo_floor(currentTimeStamp / ZO_ONE_DAY_IN_SECONDS)
    local timeStamp = days * ZO_ONE_DAY_IN_SECONDS + relativeDay * ZO_ONE_DAY_IN_SECONDS
    return timeStamp
end
...
local TRADITIONAL_WEEK = {
    [0] = 3 * ZO_ONE_DAY_IN_SECONDS, -- Thursday
    [1] = 4 * ZO_ONE_DAY_IN_SECONDS, -- Friday
    [2] = 5 * ZO_ONE_DAY_IN_SECONDS, -- Saturday
    [3] = 6 * ZO_ONE_DAY_IN_SECONDS, -- Sunday
    [4] = 0, -- Monday
    [5] = 1 * ZO_ONE_DAY_IN_SECONDS, -- Tuesday
    [6] = 2 * ZO_ONE_DAY_IN_SECONDS, -- Wednesday
}

-- EU: Tuesdays - 14:00 UTC, 10am EDT / 9am EST
-- NA: Tuesdays - 19:00 UTC, 3pm EDT / 2pm EST
local NEW_WEEK = {
    [0] = 1 * ZO_ONE_DAY_IN_SECONDS, -- Thursday
    [1] = 2 * ZO_ONE_DAY_IN_SECONDS, -- Friday
    [2] = 3 * ZO_ONE_DAY_IN_SECONDS, -- Saturday
    [3] = 4 * ZO_ONE_DAY_IN_SECONDS, -- Sunday
    [4] = 5 * ZO_ONE_DAY_IN_SECONDS, -- Monday
    [5] = 6 * ZO_ONE_DAY_IN_SECONDS, -- Tuesday
    [6] = 0, -- Wednesday
}

--- Returns the UTC timestamp for the start of the trading week ---
function ArkadiusTradeTools:GetStartOfWeek(relativeWeek, useTradeWeek)
    ...
    local result = days * ZO_ONE_DAY_IN_SECONDS
    ...
    if useTradeWeek then
        if megaserver == "EU Megaserver" and USE_TRADITIONAL_WEEK_START or (megaserver ~= "EU Megaserver" and not USE_TRADITIONAL_WEEK_START) then
            ...
            local hoursLeftThisWeek = zo_floor(secondsLeftThisWeek / ZO_ONE_HOUR_IN_SECONDS)
            if hoursLeftThisWeek < 5 then
                result = result + SECONDS_IN_WEEK
            end
            result = result - 5 * ZO_ONE_HOUR_IN_SECONDS
        elseif megaserver ~= "EU Megaserver" then
            -- NA Traditional
            local secondsGoneThisWeek = currentTimeStamp - self:GetStartOfWeek(0)
            local hoursGoneThisWeek = zo_floor(secondsGoneThisWeek / ZO_ONE_HOUR_IN_SECONDS)
            if hoursGoneThisWeek < 1 then
                result = result - SECONDS_IN_WEEK
            end
            result = result + 1 * ZO_ONE_HOUR_IN_SECONDS
        else
            -- EU Not Traditional
            local secondsLeftThisWeek = self:GetStartOfWeek(1) - currentTimeStamp
            local hoursLeftThisWeek = zo_floor(secondsLeftThisWeek / ZO_ONE_HOUR_IN_SECONDS)
            ...
            result = result - 10 * ZO_ONE_HOUR_IN_SECONDS
        end
    end
    return result
end
...
function ArkadiusTradeTools:LocalizeDezimalNumber(number)
    local left, num, right = zo_strmatch(number, "^([^%d]*%d)(%d*)(.-)$")
