...
function ArkadiusTradeToolsPurchasesList:Initialize(control)
    ...
    ZO_ScrollList_AddDataType(self.list, 1, "ArkadiusTradeToolsPurchasesRow", 32, function(control, data)
...
function ArkadiusTradeToolsPurchasesList:SetupFilters()
    ...
    if self["buyerNameSwitch"] then
        self.Filter:SetKeyFunc(2, "buyerName", CompareStringsFuncs[useSubStrings])
    else
        self.Filter:SetKeyFunc(2, "buyerName", nil)
    end

    if self["sellerNameSwitch"] then
        self.Filter:SetKeyFunc(2, "sellerName", CompareStringsFuncs[useSubStrings])
    else
...
function ArkadiusTradeToolsPurchases:Initialize()
    ...
    -----------------------
    ArkadiusTradeTools:RegisterCallback(ArkadiusTradeTools.EVENTS.ON_GUILDSTORE_ITEM_BOUGHT, function(...)
        self:OnItemBought(...)
    end)
end
...
function ArkadiusTradeToolsPurchases:GetSettingsMenu()
    ...
    table.insert(settingsMenu, {
        type = "slider",
        name = L["ATT_STR_KEEP_PURCHASES_FOR_DAYS"],
        min = 1,
        max = 30,
        getFunc = function()
...
local function onAddOnLoaded(eventCode, addonName)
    ...
    if Settings.filters.buyerName == nil then
        Settings.filters.buyerName = true
    end
